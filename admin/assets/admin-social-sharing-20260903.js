(() => {
    ...
    const views = {
        ...
        social: ['Sdílení na sítích', 'Obrázek a text odkazu pro každou stránku.'],
    ...
    function friendlyLabel(element, property) {
        ...
        if (element.classList.contains('gw-kicker')) return 'Popisek hodnocení';
        if (element.matches('[data-google-review-score]')) return 'Číselné hodnocení';
        if (element.matches('[data-google-review-count]')) return 'Souhrn počtu recenzí';
        if (element.classList.contains('gw-name')) return 'Jméno recenzenta';
    ...
    function renderInspector() {
        ...
        if (state.selectedGroup === 'Reference') {
            const guide = document.createElement('div');
            guide.className = 'inspector-guide';
            guide.innerHTML = '<strong>Souhrn a jednotlivé recenze</strong><p>Nahoře upravíte nadpis, hodnocení i text „Na základě 50+ recenzí“. Jednotlivé recenze jsou níže přehledně podle jména klienta.</p>';
            fields.append(guide);
        }
    ...
    function renderVideos() {
        ...
        state.videos.forEach((item, index) => {
            ...
            const project = document.createElement('div'); project.className = 'portfolio-project'; const title = document.createElement('strong'); title.textContent = item.title; const label = document.createElement('small'); label.textContent = item.label || 'Bez popisku';
            const homepage = document.createElement('label'); homepage.className = 'portfolio-home-toggle';
            const homepageInput = document.createElement('input'); homepageInput.type = 'checkbox'; homepageInput.checked = Boolean(item.homepage);
            const homepageText = document.createElement('span'); homepageText.textContent = 'Homepage';
            homepage.append(homepageInput, homepageText); project.append(title, label, homepage);
            homepageInput.addEventListener('change', () => {
                const selectedCount = state.videos.reduce((count, entry, itemIndex) => count + (itemIndex === index ? homepageInput.checked : Boolean(entry.homepage) ? 1 : 0), 0);
                if (selectedCount > 6) { homepageInput.checked = false; toast('Na homepage lze vybrat nejvýše 6 realizací.', true); return; }
                state.videos[index].homepage = homepageInput.checked; state.videosDirty = true; $('#saveVideos').disabled = false; renderVideos();
            });
        ...
        const homepageCount = state.videos.filter(item => item.homepage).length;
        $('#portfolioCount').textContent = `${state.videos.length} ${state.videos.length === 1 ? 'projekt' : state.videos.length < 5 ? 'projekty' : 'projektů'} · ${homepageCount} vybraných na homepage`;
    }
    ...
    async function loadVideos() {
        try {
            const data = await api('api/videos.php'); state.videos = (data.items || []).map((item, index) => ({ ...item, homepage: typeof item.homepage === 'boolean' ? item.homepage : index < 6 }));
    ...
    function openVideoEditor(index = -1) {
        const item = index >= 0 ? state.videos[index] : { title: '', label: '', categories: [], video: '', image: '', alt: '', homepage: false };
        $('#videoEditIndex').value = String(index); $('#videoTitle').value = item.title || ''; $('#videoLabel').value = item.label || ''; $('#videoHomepage').checked = Boolean(item.homepage); $('#videoUrl').value = item.video || ''; $('#videoImage').value = item.image || ''; $('#videoAlt').value = item.alt || '';
    ...
    $('#videoEditor').addEventListener('submit', async event => {
        ...
        const homepage = $('#videoHomepage').checked;
        const selectedElsewhere = state.videos.filter((entry, itemIndex) => itemIndex !== index && entry.homepage).length;
        if (homepage && selectedElsewhere >= 6) { toast('Na homepage lze vybrat nejvýše 6 realizací. Nejprve některou jinou odškrtněte.', true); return; }
        const item = { id: index >= 0 ? state.videos[index].id : id, title, label: $('#videoLabel').value.trim(), categories, video: $('#videoUrl').value.trim(), image: $('#videoImage').value.trim(), alt: $('#videoAlt').value.trim() || title, homepage };
    ...
    function socialValues() {
        return {
            ...
            image: $('#socialImage').value.trim(),
            imageAlt: $('#socialImageAlt').value.trim()
        };
    }
    function updateSocialPreview() {
        const values = socialValues();
        $('#socialTitleCount').textContent = `${values.title.length} / 110`;
        $('#socialDescriptionCount').textContent = `${values.description.length} / 240`;
        $('#socialPreviewTitle').textContent = values.title || 'Titulek sociální karty';
        $('#socialPreviewDescription').textContent = values.description || 'Popis stránky se zobrazí zde.';
        $('#socialPreviewImage').src = values.image || '/assets/og/homepage.jpg';
        $('#socialPreviewImage').alt = values.imageAlt || '';
        const original = state.socialOriginal || {};
        state.socialDirty = ['title', 'description', 'image', 'imageAlt'].some(key => values[key] !== String(original[key] || ''));
        $('#saveSocial').disabled = !state.socialDirty;
        $('#socialPreviewStatus').textContent = state.socialDirty ? 'Neuložené změny' : (state.socialEditing?.customized ? 'Vlastní nastavení' : 'Doporučený formát');
        $('#socialPreviewStatus').classList.toggle('is-dirty', state.socialDirty);
    }
    function showSocialItem(item) {
        state.socialEditing = item;
        state.socialOriginal = { ...item.current };
        $('#socialTitle').value = item.current.title || '';
        $('#socialDescription').value = item.current.description || '';
        $('#socialImage').value = item.current.image || '';
        $('#socialImageAlt').value = item.current.imageAlt || '';
        $('#socialOpenPage').href = item.url;
        $('#socialPreviewDomain').textContent = new URL(item.url).hostname.toUpperCase();
        updateSocialPreview();
    }
    async function loadSocial(force = false) {
        try {
            if (!state.socialItems.length || force) {
                const data = await api('api/social.php');
                state.socialItems = data.items || [];
                $('#socialPage').innerHTML = state.socialItems.map(item => `<option value="${escapeHtml(item.key)}">${escapeHtml(item.label)}</option>`).join('');
            }
            const selected = state.socialItems.find(item => item.key === $('#socialPage').value) || state.socialItems[0];
            if (selected) { $('#socialPage').value = selected.key; showSocialItem(selected); }
        } catch (error) { toast(error.message, true); }
    }
    $('#socialPage').addEventListener('change', event => {
        if (state.socialDirty && !confirm('Zahodit neuložené změny této sociální karty?')) { event.target.value = state.socialEditing?.key || ''; return; }
        const item = state.socialItems.find(entry => entry.key === event.target.value);
        if (item) showSocialItem(item);
    });
    ['socialTitle', 'socialDescription', 'socialImage', 'socialImageAlt'].forEach(id => $('#' + id).addEventListener('input', updateSocialPreview));
    $('#chooseSocialImage').addEventListener('click', () => openMediaPicker(url => { $('#socialImage').value = url; updateSocialPreview(); }));
    $('#uploadSocialImage').addEventListener('change', async event => { if (!event.target.files[0]) return; const item = await uploadFile(event.target.files[0]); if (item) { $('#socialImage').value = item.url; updateSocialPreview(); } event.target.value = ''; });
    $('#resetSocial').addEventListener('click', () => {
        const values = state.socialEditing?.recommended || state.socialEditing?.defaults;
        if (!values) return;
        $('#socialTitle').value = values.title || '';
        $('#socialDescription').value = values.description || '';
        $('#socialImage').value = values.image || '';
        $('#socialImageAlt').value = values.imageAlt || '';
        updateSocialPreview();
    });
    $('#socialEditor').addEventListener('submit', async event => {
        event.preventDefault();
        if (!state.socialEditing) return;
        try {
            const result = await api('api/social.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: state.socialEditing.key, ...socialValues() }) });
            state.socialEditing.current = { ...result.item };
            delete state.socialEditing.current.key;
            state.socialEditing.customized = true;
            state.socialOriginal = { ...state.socialEditing.current };
            state.socialDirty = false;
            updateSocialPreview();
    ...
    window.addEventListener('beforeunload', event => { const contentDirty = [...state.current].some(([key, value]) => value !== state.saved.get(key)); if (contentDirty || state.videosDirty || state.boothDirty || state.socialDirty) { event.preventDefault(); event.returnValue = ''; } });
    init();
})();
